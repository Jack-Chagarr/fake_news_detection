...
from .models.bert import BERTRegression, AlbertForMaskedLMOptimized, BertForMaskedLMOptimized, DistilBertForMaskedLMOptimized,ErnieForMaskedLMOptimized 
    ...
    def corpus_to_dataset(self, corpus: Corpus) -> SimpleDataset:
        ...
        for sent_idx, sent in enumerate(corpus.values()):
            ...
            if self._add_special:
                ...
                tokens_original = [self._vocab.cls_token] + self._tokenizer(sent)["input_ids"] + [self._vocab.sep_token]
            else:
                tokens_original = [self._vocab.cls_token] + self._tokenizer(sent)["input_ids"]
            ids_original = np.array(self._tokenizer.convert_tokens_to_ids(tokens_original))
...
class MLMScorerPT(BaseScorer):
    ...
    def _check_support(model) -> bool:
        return True
        #return isinstance(model, transformers.ErnieModel) or isinstance(model, transformers.BertForMaskedLM) or isinstance(model, AlbertForMaskedLMOptimized) or isinstance(model, BertForMaskedLMOptimized) or isinstance(model, DistilBertForMaskedLMOptimized or isinstance(model, transformers.XLMWithLMHeadModel))
            ...
            for ctx_idx, (sent_idxs, token_ids, valid_length, masked_positions, token_masked_ids, normalization) in enumerate((batch,)):
                ...
                with torch.no_grad():
                    ...
                    if isinstance(self._model.module, AlbertForMaskedLMOptimized) or \
                        ...
                        # Reindex to only get the distributions at the masked positions (batch_size, config.vocab_size)
                        out = out[0][list(range(split_size)),masked_positions.reshape(-1),:]
                    elif isinstance(self._model.module, transformers.ErnieForMaskedLM):
                        alen = torch.arange(token_ids.shape[1], dtype=torch.long)
                        alen = alen.to(ctx)
                        mask = alen < valid_length[:, None]
                        out = self._model(input_ids=token_ids, attention_mask=mask)
