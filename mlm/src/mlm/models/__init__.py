...
from .bert import BERTRegression, AlbertForMaskedLMOptimized, BertForMaskedLMOptimized, DistilBertForMaskedLMOptimized, ErnieForMaskedLMOptimized
...
def get_pretrained(ctxs: List[mx.Context], name: str = 'bert-base-en-uncased', params_file: Optional[Path] = None, cased: bool = False, finetune: bool = False, regression: bool = False, freeze: int = 0, root: Optional[Path] = None) -> Tuple[Block, nlp.Vocab, nlp.data.BERTTokenizer]:
    ...
    if name not in SUPPORTED:
        ...
        if model_name.startswith('albert-'):
            ...
            # print("hello")
            # if params_file is None:
            #     model, loading_info = transformers.BertModel.from_pretrained("nghuyong/ernie-2.0-base-en", output_loading_info=True)
            # else:
            #     model, loading_info = transformers.BertModel.from_pretrained("nghuyong/ernie-2.0-base-en", output_loading_info=True)
              
            # tokenizer = BertTokenizer.from_pretrained("nghuyong/ernie-2.0-base-en")
            # vocab = None
            if params_file is None:
                model, loading_info = ErnieForMaskedLMOptimized.from_pretrained(model_fullname, output_loading_info=True)
            else:
                model, loading_info = ErnieForMaskedLMOptimized.from_pretrained(params_file, output_loading_info=True)

            tokenizer = transformers.BertTokenizer.from_pretrained("nghuyong/ernie-2.0-base-en")
            vocab = None


            # kwargs = {
            #     'dataset_name': 'baidu_ernie_uncased',
            #     'pretrained': True,
            #     'ctx': ctxs,
            #     'use_pooler': False,
            #     'use_decoder': False,
            ...
            # _, vocab = get_model('ernie_12_768_12', **kwargs)
    ...
    # e.g., 'bert-base-en-uncased-owt', 'gpt2-117m-en-cased'
        ...
        if model_name == 'bert'and size != 'base_bertpr':
            ...
            model_fullname = 'ernie_12_768_12'
